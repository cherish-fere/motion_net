...
def train(path, phase, batch_size, height, width, channels, num_classes, epochs, init_lr):
	...
	with tf.variable_scope("input_container"):
		x = tf.placeholder(dtype=tf.float32, shape=[batch_size, height, width, channels])
		y = tf.placeholder(dtype=tf.int64, shape=[batch_size, height, width, 1])  # non-one-hot
		prob = tf.placeholder(tf.float32)
		lr = tf.placeholder(dtype=tf.float32)

	with tf.variable_scope("", reuse=tf.AUTO_REUSE):
		seg_logit, rec_logit, seg_var, rec_var = multi_fcn(x, num_classes, prob)

	# segmentation loss
	seg_loss_logit = softmax_loss(logits=seg_logit, labels=y, is_regularize=False)
	# reconstruction loss
	rec_loss_logit = mse_loss(labels=y, rec_logits=rec_logit, images=x, is_regularize=False)
	...
	seg_optimizer = tf.train.AdamOptimizer(learning_rate=lr, beta1=0.5).minimize(seg_loss_logit, var_list=seg_var)
	rec_optimizer = tf.train.AdamOptimizer(learning_rate=lr, beta1=0.5).minimize(rec_loss_logit, var_list=rec_var)
	...
	with tf.Session(config=config) as sess:
		...
		# seq_writer = tf.summary.FileWriter(train_log_path, sess.graph)
		...
		for epoch in range(epochs):
			...
			for step in range(train_steps):
				...
				feed = {x: train_data_batch['n_image'], y: train_data_batch['label'], prob: 0.85,  lr: learning_rate}
				sess.run(local_init)

				_, _, _ = sess.run([seg_optimizer, rec_optimizer, miou_updates], feed_dict=feed)
